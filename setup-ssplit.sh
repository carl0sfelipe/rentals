...
http {
    ...
    server {
        ...
        server_name DOMAIN_PROD_PLACEHOLDER;

        ssl_certificate /etc/letsencrypt/live/DOMAIN_PROD_PLACEHOLDER/fullchain.pem;
        ssl_certificate_key /etc/letsencrypt/live/DOMAIN_PROD_PLACEHOLDER/privkey.pem;
    ...
    server {
        ...
        location / {
            return 200 'Use https://DOMAIN_PROD_PLACEHOLDER';
...
sed -i "s|DOMAIN_PROD_PLACEHOLDER|$DOMAIN_PROD|g" nginx.conf
...
if grep -q "/etc/letsencrypt:/etc/letsencrypt" docker-compose.production.yml; then
    ...
    echo "⚠️  IMPORTANTE: Adicione esta linha ao docker-compose.production.yml"
    echo "   Na seção nginx > volumes, adicione:"
...
CRON_CMD="0 0 * * * certbot renew --quiet && docker compose -f $(pwd)/docker-compose.production.yml restart nginx"
...
echo ""
echo "1. Adicionar FRONTEND_URL no .env:"
echo "   echo 'FRONTEND_URL_PROD=https://seu-dominio-prod.vercel.app' >> .env"
echo "   docker compose -f docker-compose.production.yml restart api-prod"
echo ""
echo "2. Na Vercel, configure:"
echo "   VITE_API_URL=https://$DOMAIN_PROD"
echo ""
echo "3. Faça redeploy na Vercel"
echo ""
echo "📝 Suas URLs:"
echo "   PROD: https://$DOMAIN_PROD"
if [ "$GET_DEV" = "s" ] || [ "$GET_DEV" = "S" ]; then
