...
const prismaMock = {
    user: {
        findUnique: vitest_1.vi.fn(async (args) => {
...
const jwtMock = {
    signAsync: vitest_1.vi.fn(async () => 'signed-jwt-token'),
};
const hashFn = vitest_1.vi.fn(async (plain) => `hashed:${plain}`);
const compareFn = vitest_1.vi.fn(async (plain, hashed) => hashed === `hashed:${plain}`);
class AuthServiceTestWrapper extends auth_service_1.AuthService {
    constructor() {
        super(prismaMock, jwtMock);
        this.hashPassword = hashFn;
        this.comparePassword = compareFn;
    }
}
(0, vitest_1.describe)('AuthService (unit)', () => {
    ...
    (0, vitest_1.beforeEach)(() => {
        ...
        service = new AuthServiceTestWrapper();
    });
    (0, vitest_1.describe)('register', () => {
        (0, vitest_1.it)('deve criar novo usuário quando email não existe', async () => {
            ...
            (0, vitest_1.expect)(hashFn).toHaveBeenCalledTimes(1);
        ...
        (0, vitest_1.it)('deve chamar função de hash de senha', async () => {
            ...
            (0, vitest_1.expect)(hashFn).toHaveBeenCalledWith('secret');
        });
