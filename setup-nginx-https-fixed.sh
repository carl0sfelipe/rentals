#!/bin/bash

# Script de Instalação Nginx + HTTPS para Ambiente Dev (CORRIGIDO)
# Domain: dev-3000-45-55-95-48.sslip.io
# IP: 45.55.95.48
# Backend Port: 3000

set -e  # Exit on error

echo "=================================================="
echo "🚀 Setup Nginx + HTTPS para Ambiente Dev"
echo "=================================================="
echo ""

# Verificar se está rodando como root
if [ "$EUID" -ne 0 ]; then
    echo "❌ Por favor, execute como root (sudo)"
    exit 1
fi

echo "✅ Executando como root"
echo ""

# 1. Atualizar sistema
echo "📦 Atualizando sistema..."
apt update -qq
echo "✅ Sistema atualizado"
echo ""

# 2. Verificar/Instalar Nginx
echo "📦 Verificando Nginx..."
if ! command -v nginx &> /dev/null; then
    apt install nginx -y -qq
    systemctl enable nginx
fi
echo "✅ Nginx pronto"
echo ""

# 3. Configurar Firewall
echo "🔥 Configurando Firewall (UFW)..."
ufw --force enable
ufw allow 22/tcp
ufw allow 80/tcp
ufw allow 443/tcp
echo "✅ Firewall configurado"
echo ""

# 4. Instalar Certbot
echo "🔐 Instalando Certbot..."
if ! command -v certbot &> /dev/null; then
    apt install certbot python3-certbot-nginx -y -qq
fi
echo "✅ Certbot instalado"
echo ""

# 5. Criar configuração do Nginx (APENAS HTTP primeiro)
echo "📝 Criando configuração inicial do Nginx (HTTP apenas)..."
cat > /etc/nginx/sites-available/dev-api << 'EOF'
# Nginx Configuration for Development Environment (sslip.io)
# Domain: dev-3000-45-55-95-48.sslip.io → http://localhost:3000
# Certbot will add HTTPS configuration automatically

server {
    listen 80;
    listen [::]:80;
    server_name dev-3000-45-55-95-48.sslip.io;

    # CORS Configuration for Vercel frontend
    add_header Access-Control-Allow-Origin "https://rentals-dev-zeta.vercel.app" always;
    add_header Access-Control-Allow-Methods "GET, POST, PUT, DELETE, PATCH, OPTIONS" always;
    add_header Access-Control-Allow-Headers "Content-Type, Authorization, X-Requested-With" always;
    add_header Access-Control-Allow-Credentials "true" always;

    # Handle preflight requests
    if ($request_method = 'OPTIONS') {
        return 204;
    }

    # Logging
    access_log /var/log/nginx/dev-api-access.log;
    error_log /var/log/nginx/dev-api-error.log;

    # Max body size for uploads
    client_max_body_size 10M;

    # Proxy configuration to Node.js application on port 3000
    location / {
        proxy_pass http://localhost:3000;
        proxy_http_version 1.1;

        # Proxy Headers
        proxy_set_header Host $host;
        proxy_set_header X-Real-IP $remote_addr;
        proxy_set_header X-Forwarded-For $proxy_add_x_forwarded_for;
        proxy_set_header X-Forwarded-Proto $scheme;
        proxy_set_header X-Forwarded-Host $host;
        proxy_set_header X-Forwarded-Port $server_port;

        # WebSocket support (if needed)
        proxy_set_header Upgrade $http_upgrade;
        proxy_set_header Connection "upgrade";

        # Timeouts
        proxy_connect_timeout 60s;
        proxy_send_timeout 60s;
        proxy_read_timeout 60s;

        # Buffering
        proxy_buffering on;
        proxy_buffer_size 4k;
        proxy_buffers 8 4k;
        proxy_busy_buffers_size 8k;

        # Error handling
        proxy_next_upstream error timeout invalid_header http_500 http_502 http_503 http_504;
    }

    # Health check endpoint (optional)
    location /health {
        proxy_pass http://localhost:3000/health;
        access_log off;
    }
}
EOF
echo "✅ Configuração HTTP criada em /etc/nginx/sites-available/dev-api"
echo ""

# 6. Ativar site
echo "🔗 Ativando site..."
ln -sf /etc/nginx/sites-available/dev-api /etc/nginx/sites-enabled/
rm -f /etc/nginx/sites-enabled/default
echo "✅ Site ativado"
echo ""

# 7. Testar configuração
echo "🧪 Testando configuração do Nginx..."
nginx -t
echo "✅ Configuração válida"
echo ""

# 8. Reiniciar Nginx
echo "🔄 Reiniciando Nginx..."
systemctl restart nginx
echo "✅ Nginx reiniciado"
echo ""

# 9. Verificar se backend está rodando
echo "🔍 Verificando se backend está rodando na porta 3000..."
if curl -s http://localhost:3000 > /dev/null 2>&1; then
    echo "✅ Backend está respondendo na porta 3000"
else
    echo "⚠️  ATENÇÃO: Backend não está respondendo na porta 3000!"
    echo "   Por favor, inicie sua aplicação Node.js antes de continuar."
    echo ""
    read -p "Pressione Enter quando o backend estiver rodando..."
fi

# 10. Testar HTTP (antes do SSL)
echo "🧪 Testando acesso HTTP..."
if curl -s http://dev-3000-45-55-95-48.sslip.io > /dev/null 2>&1; then
    echo "✅ HTTP funcionando!"
else
    echo "⚠️  HTTP não está acessível ainda. Aguarde alguns segundos..."
    sleep 5
fi
echo ""

# 11. Obter certificado SSL com Certbot
echo "🔐 Obtendo certificado SSL com Let's Encrypt..."
echo "   Certbot vai modificar automaticamente o arquivo de configuração."
echo ""
certbot --nginx -d dev-3000-45-55-95-48.sslip.io --non-interactive --agree-tos --register-unsafely-without-email --redirect || {
    echo ""
    echo "⚠️  Certbot automático falhou. Tentando com e-mail..."
    read -p "Digite seu e-mail: " email
    certbot --nginx -d dev-3000-45-55-95-48.sslip.io --non-interactive --agree-tos --email "$email" --redirect || {
        echo ""
        echo "❌ Certbot falhou. Execute manualmente:"
        echo "   sudo certbot --nginx -d dev-3000-45-55-95-48.sslip.io"
        exit 1
    }
}
echo ""
echo "✅ Certificado SSL obtido com sucesso!"
echo ""

# 12. Testar renovação automática
echo "🔄 Testando renovação automática..."
certbot renew --dry-run
echo "✅ Renovação automática configurada"
echo ""

# 13. Verificar HTTPS
echo "🧪 Testando HTTPS..."
sleep 2
if curl -s https://dev-3000-45-55-95-48.sslip.io > /dev/null 2>&1; then
    echo "✅ HTTPS funcionando perfeitamente!"
else
    echo "⚠️  HTTPS ainda não está acessível. Pode levar alguns segundos."
fi
echo ""

echo "=================================================="
echo "✅ INSTALAÇÃO CONCLUÍDA COM SUCESSO!"
echo "=================================================="
echo ""
echo "🎉 Sua API agora está disponível em:"
echo "   https://dev-3000-45-55-95-48.sslip.io"
echo ""
echo "🔧 Configurações:"
echo "   - Nginx rodando nas portas 80 e 443"
echo "   - Proxy para http://localhost:3000"
echo "   - SSL válido (Let's Encrypt)"
echo "   - CORS configurado para: https://rentals-dev-zeta.vercel.app"
echo "   - Redirect automático HTTP → HTTPS"
echo ""
echo "📝 Próximos passos:"
echo "   1. Acesse: https://dev-3000-45-55-95-48.sslip.io no navegador"
echo "   2. Verifique o cadeado verde 🔒"
echo "   3. Atualize sua URL no front-end Vercel para:"
echo "      https://dev-3000-45-55-95-48.sslip.io"
echo ""
echo "🐛 Troubleshooting:"
echo "   - Logs do Nginx: sudo tail -f /var/log/nginx/dev-api-error.log"
echo "   - Status do Nginx: sudo systemctl status nginx"
echo "   - Testar backend: curl http://localhost:3000"
echo "   - Ver certificados: sudo certbot certificates"
echo ""
echo "=================================================="
